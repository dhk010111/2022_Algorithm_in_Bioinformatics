// 다중서열정렬을 시도할 sequence를 생성하는 코드 (4주차 실습 코드 재사용)
...
//문제 1 : 길이 N의 무작위 염기서열을 생성하여 origin.txt로 저장하는 함수
...
//문제 2 : origin과 10퍼센트의 차이를 보이는 mutation.txt를 생성하는 함수
void makemutation(int n, int num)
{
    ...
    //origin.txt의 시퀀스 중 SNPpos에 해당하는 부분을 다른 시퀀스로 변경한다. (n/10개)
    //nextSNP는 다음으로 변경해야할 위치를 나타낸다.
